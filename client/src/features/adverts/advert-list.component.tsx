...
export const AdvListComponent = ({ item, onDelete, getData, favorites, setFavorites }: Props) => {
	...
	const handleResponse = async () => {
		try {
			const response = await fetchApi.api.itemadvertisementsResponse({
			...
			if (response) {
				if (response.status === 200) {
					dispatch(self());
					toast.success('Вы откликнулись на объявление, проверьте почту');
	...
	const handleAddCollectionToFavorites = async (id: string) => {
		try {
			...
			if (response.status === 200) {
				...
				getUserFavorites();
				getData();
				toast.success('Объявление в избранном');
			} else if (response.status === 401) {
				toast.error('Не авторизован');
			} else {
				toast.error('Ошибка:' + response.statusText);
	...
	const handleDeleteCollectionFromFavorites = async (id: string) => {
		try {
			...
			if (response.status === 200) {
				...
				getUserFavorites();
				getData();
				toast.success('Объявление убрано из избранного');
			} else if (response.status === 401) {
				toast.error('Не авторизован');
			} else {
				toast.error('Ошибка:' + response.statusText);
		...
		<div className="flex h-fit w-full flex-row items-center gap-[20px] rounded-lg bg-primary-green px-[30px] py-[10px] scrollbar-none">
			...
			<div className="flex w-full flex-col flex-wrap gap-[10px] self-start pt-[30px] text-[20px]">
				...
				<div className="flex flex-row gap-[10px]">
					<p className="font-bold">Название предмета:</p>
				...
				<div className="flex max-h-[300px] max-w-[700px] flex-wrap overflow-y-auto text-wrap">
					...
					<p
						...
						title={item.advertisement_description}
						style={{ wordWrap: 'break-word', whiteSpace: 'pre-wrap' }}
					>
						{item.advertisement_description}
					</p>
				</div>
